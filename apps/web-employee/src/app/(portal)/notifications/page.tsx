...
import { Card, CardContent, Button, formatDate } from '@kezad/ui';
...
export default function NotificationsPage() {
  ...
  const { data, isLoading } = useQuery<{ data: Notification[] }>({
    queryKey: ['emp-notifications'],
    queryFn: () => api.get('/notifications').then((r) => r.data),
  });

  const markReadMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/notifications/${id}/read`),
    onSuccess: () => { void qc.invalidateQueries({ queryKey: ['emp-notifications'] }); },
  });
    ...
    <div className="animate-fade-in">
      <div className="kezad-page-header">
        <div className="flex items-center gap-3">
          ...
          {unreadCount > 0 && (
            <span className="bg-primary text-white text-xs font-bold px-2.5 py-1 rounded-full">{unreadCount} new</span>
          )}
        </div>
        {unreadCount > 0 && (
          <Button variant="outline" size="sm" loading={markAllMutation.isPending} onClick={() => markAllMutation.mutate()}>
      ...
      <div className="p-8">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray-400 mt-1">System alerts and workflow notifications will appear here.</p>
          </div>
        ) : (
          <Card>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            {n.subject && <p className={`text-sm font-semibold ${!n.isRead ? 'text-gray-900' : 'text-gray-700'}`}>{n.subject}</p>}
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          ...
                          {!n.isRead && (
                            <button onClick={() => markReadMutation.mutate(n.id)} className="text-xs text-primary hover:text-primary/80 font-medium">
                              Mark as read
                            </button>
                          )}
                          {n.isRead && n.readAt && <span className="text-xs text-gray-400">Read {formatDate(n.readAt)}</span>}
                        </div>
                      </div>
                    </div>
                  );
